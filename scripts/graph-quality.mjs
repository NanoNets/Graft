 ...
 *   - well-formed spans (`Lx-Ly`, x<=y)
 *   - unique node ids; non-empty names; kinds in the allowed set
 *   - no self-loop call edges
 * Exits non-zero with --strict when any invariant fails; ci.yml runs exactly that
 * over graft's own graph, so this file is a live gate rather than a claim.
 *
 * The vocabulary sets below duplicate src/graph/invariants.ts on purpose (see the
 * note there: the CLI report has to work against a stale or missing dist/). They
 * are exported so test/graph-quality-script.test.ts can assert the two copies —
 * and the `Kind`/`Relation`/`Confidence` unions they mirror — have not drifted;
 * before that guard, adding a kind to types.ts made this script report a
 * false-positive `bad kind` on every real graph.
 */
import { readFileSync, existsSync } from "node:fs";
import { join, resolve } from "node:path";
import { fileURLToPath } from "node:url";

export const KINDS = new Set(["file","class","function","method","interface","type","enum","struct","module","constant","variable"]);
export const RELATIONS = new Set(["contains","calls","imports","references","implements","extends"]);
export const CONFIDENCE = new Set(["lsp_resolved","lsp_dispatch","extracted","inferred"]);

/** Shape + invariant report for an already-parsed graph object. Pure. */
export function analyze(g, wpath) {
...
function main() {
  ...
  const report = analyze(JSON.parse(readFileSync(wpath, "utf8")), wpath);
...
// Only run the CLI when invoked as one — the drift test imports this module for
// its constants, and a top-level `process.exit(2)` would kill the test runner.
