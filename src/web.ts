 ...
 *
 * The request handling is packaged as {@link createWebApp} so the unified
 * `context-graph serve` command can mount it alongside the MCP-over-HTTP
 * endpoint on one port, sharing a single engine and watcher. Run directly
 * (`context-graph-web`) it keeps its original standalone behavior.
 *
...
import { resolveConfig, type ResolvedConfig } from "./ai/providers.js";
...
/** Which extraction/answer/embedding providers are live, for the UI to display honestly. */
...
export interface WebAppOptions {
  /** The (shared) engine backing the UI. */
  engine: ContextGraphEngine;
  /** Resolved config; defaults to {@link resolveConfig}. */
  cfg?: ResolvedConfig;
  /** Host the server binds to — decides whether an access token is required. */
  host: string;
  /** Port the server listens on — used only to render share links. */
  port: number;
  /** Lazily create (or return) the watcher used for in-process auto-watch. */
  ensureWatcher: () => GraphWatcher;
  /** Peek at the live watcher without creating one (for status endpoints). */
  getWatcher?: () => GraphWatcher | undefined;
}

export interface WebApp {
  /** node:http request handler for the UI + /api/* endpoints. */
  ...
  webToken?: string;
  /** Path of the human-readable share-link file, if written. */
  sharePath?: string;
  /** Which extraction/answer/embedding providers are live. */
  providerStatus: () => ProviderStatus;
}

/**
 * Build the web request handler over a given engine. The handler is transport
 * agnostic: `context-graph-web`'s `main()` wraps it in its own HTTP server, and
 * `context-graph serve` mounts it behind a router that also serves `/mcp`.
 */
export function createWebApp(opts: WebAppOptions): WebApp {
  const { engine, host, port, ensureWatcher } = opts;
  const cfg = opts.cfg ?? resolveConfig();
  const getWatcher = opts.getWatcher ?? (() => undefined);
...
/** Standalone `context-graph-web` entry point. */
async function main(): Promise<void> {
  ...
  const ensureWatcher = (): GraphWatcher =>
    (webWatcher ??= new GraphWatcher(engine, {
      onEvent: (event) => {
  ...
  server.listen(PORT, HOST, () => {
    const p = app.providerStatus();
    console.log(`Context Graph web UI  →  http://localhost:${PORT}`);
    if (app.exposed && app.webToken) {
      const lan = lanAddress();
      console.log(`  access token: ${app.webToken}`);
      ...
      if (lan) console.log(`  share with your team:  http://${lan}:${PORT}/#token=${app.webToken}`);
      else console.log(`  share:  http://<this-machine>:${PORT}/#token=${app.webToken}`);
      ...
      if (app.sharePath) console.log(`  share link also saved to:  ${app.sharePath}`);
    }
...
// Run the standalone server only when invoked directly (as the
// `context-graph-web` bin), not when imported by `serve`.
