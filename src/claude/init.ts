...
export function runInit(dir: string, opts: { build?: boolean; cliPath?: string; mcp?: boolean; hooks?: boolean } = {}): InitResult {
  ...
  const { merged, warnings } = mergeGraftSettings(existing, { hooks: opts.hooks });
  ...
  return { settingsPath, shims, skill: skillPath, mcp, warnings, built };
}
